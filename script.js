...
function toggleNamePrompt() {
  const cloud = document.getElementById("namePrompt");
  cloud.style.display = cloud.style.display === "block" ? "none" : "block";
}
...
function updateHeader() {
  ...
  if (name) {
    authArea.innerText = name;
  } else if (localStorage.getItem("userId")) {
    authArea.innerText = "User";
  } else {
    authArea.innerText = "Login";
  }
}
...
window.onload = () => {
  ...
  // 🔥 SHOW NAME CLOUD IF LOGGED IN BUT NO DISPLAY NAME
  if (localStorage.getItem("userId") && !localStorage.getItem("displayName")) {
    document.getElementById("namePrompt").style.display = "block";
  }
};
